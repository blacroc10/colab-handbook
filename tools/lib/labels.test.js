...
test('the convention set is exactly the thirteen labels §9 provisions, in canonical order', () => {
  assert.deepStrictEqual(
    conventionLabelNames(),
    ['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only'],
  );
...
test('a repo with every label is not flagged', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only', 'bug']),
    [],
  );
});
...
test('the readiness label absent is reported — the exact gap that silently un-fills the column', () => {
  assert.deepStrictEqual(
    ...
    ['deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only'],
  );
});
...
test('missing preserves canonical order regardless of the input order', () => {
  assert.deepStrictEqual(missingConventionLabels(['epic', 'agent-filed']), ['in-progress', 'deps-checked', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']);
});

test('a repo missing only epic (adopted before #78) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['epic'],
  );
});
...
test('a repo missing only needs-decision (renamed from needs-ruling, #75/#122) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['needs-decision'],
  );
});
...
test('a repo missing only decision-recorded (adopted before #121) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['decision-recorded'],
  );
});
...
test('a repo missing only needs-plan (adopted before #94) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['needs-plan'],
  );
});
...
test('a repo missing only migration-granted (adopted before #98) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['migration-granted'],
  );
});
...
test('a repo missing only ci-granted (adopted before #105) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    ['ci-granted'],
  );
});
...
test('a repo missing only the delivery:* set (adopted before #112) is flagged for exactly that gap', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted']),
...
test('empty / null / undefined input reports the whole set (a bare repo, or unread labels)', () => {
  const all = ['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only'];
...
test('a repo missing graph-empty is never reported by missingConventionLabels — it is not in the set', () => {
  assert.deepStrictEqual(
    missingConventionLabels(['in-progress', 'deps-checked', 'agent-filed', 'epic', 'needs-decision', 'decision-recorded', 'needs-plan', 'migration-granted', 'ci-granted', 'delivery:code', 'delivery:content', 'delivery:ops', 'delivery:docs-only']),
    [],
  );
});
